...
write.vtu(FEMobj, paste(directory.results, "mean_", name, "_overall.vtu", sep = ""))

# CONTROL mean
means[["CONTROL"]] <- method(X[index.CONTROL,], mesh, lambdas)
FEMobj <- FEM(as.numeric(means[["CONTROL"]]$fitted_nodes), FEMbasis)
write.vtu(FEMobj, paste(directory.results, "mean_", name, "_CONTROL.vtu", sep = ""))

# SCHZ mean
means[["SCHZ"]] <- method(X[index.CONTROL,], mesh, lambdas)
FEMobj <- FEM(as.numeric(means[["SCHZ"]]$fitted_nodes), FEMbasis)
write.vtu(FEMobj, paste(directory.results, "mean_", name, "_SCHZ.vtu", sep = ""))
...
save(means,
     file = paste(directory.results, "mean_estimation_", name, ".RData", sep = ""))
